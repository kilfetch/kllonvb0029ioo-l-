...
# Определяем базовую директорию для файлов бота
# Пытаемся определить директорию запуска скрипта
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.dirname(SCRIPT_DIR)  # Один уровень вверх
...
# Пути к файлам (абсолютные или относительные в зависимости от окружения)
# Пробуем использовать абсолютные пути для хостинга
if os.path.exists(os.path.join(SCRIPT_DIR, "config.json")):
    CONFIG_FILE = os.path.join(SCRIPT_DIR, "config.json")
    ADMINS_FILE = os.path.join(SCRIPT_DIR, "admins.txt")
    logger.info(f"Используем пути в директории скрипта: {CONFIG_FILE}")
else:
    # Относительные пути для локальной разработки
    CONFIG_FILE = "config.json"
    ADMINS_FILE = "admins.txt"
    logger.info(f"Используем относительные пути: {CONFIG_FILE}")
...
class APIMonitorBot:
    ...
    async def setup(self):
        ...
        # Создаем экземпляр приложения с поддержкой job_queue
    ...
    def save_config(self) -> None:
        ...
        try:
            ...
            logger.error(f"Путь к файлу конфигурации: {CONFIG_FILE}")
    ...
    def create_admins_file(self) -> None:
        ...
        if not os.path.exists(ADMINS_FILE):
            try:
                # Создаем директорию, если она не существует
                admins_dir = os.path.dirname(ADMINS_FILE)
                if admins_dir and not os.path.exists(admins_dir):
                ...
                logger.error(f"Путь к файлу админов: {ADMINS_FILE}")
                logger.error(f"Текущая директория: {os.getcwd()}")
    ...
    def load_api_url_from_config(self) -> None:
        ...
        try:
            # Проверяем разные возможные пути к config.js
            possible_paths = [
                os.path.join(BASE_DIR, "js", "config.js"),
                os.path.join(SCRIPT_DIR, "..", "js", "config.js"),
                os.path.join(os.getcwd(), "js", "config.js"),
                "js/config.js",
                "../js/config.js",
                "/home/container/js/config.js"
            ]
            
            for config_path in possible_paths:
                logger.info(f"Проверяем путь к config.js: {config_path}")
            if os.path.exists(config_path):
                    logger.info(f"Файл config.js найден: {config_path}")
    ...
    def register_handlers(self) -> None:
        ...
        self.application.add_handler(MessageHandler(filters.ChatType.GROUPS, self.handle_group_message))
    ...
    async def schedule_api_check(self) -> None:
        ...
        # Проверяем, что job_queue настроен
        if not self.application.job_queue:
            logger.warning("JobQueue не настроен. Автоматическая проверка API не будет выполняться.")
            # Показываем сообщение вместо ошибки и продолжаем работу бота
            return
        ...
        logger.info(f"Запланирована проверка API с интервалом {interval} секунд ({minutes} минут)")
            ...
            logger.error(f"Ошибка при запуске задачи проверки API: {e}")
            # Бот продолжит работать, но без автоматической проверки API
    ...
    async def check_api_job(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            logger.error(f"Ошибка при выполнении проверки API: {str(e)}")
